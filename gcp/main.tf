...
data "template_file" "jitsi-shared" {
  template = "${file("${format("%s/scripts/jitsi-shared.sh.tpl", path.module)}")}"
  vars {
    jitsi_bucket_certificates = var.jitsi_bucket_certificates
    jitsi_hostname = var.jitsi_hostname
    jitsi_jvbsecret = var.jitsi_jvbsecret # or random?
    XMPP_AUTH_PASSWORD = ""
...
data "template_file" "jitsi-meet-startup-script" {
  ...
  vars {
    bucket_certificates = var.jitsi_bucket_certificates
    jitsi_hostname = var.jitsi_hostname
    jitsi_jvbsecret = var.jitsi_jvbsecret # or random?
    XMPP_AUTH_PASSWORD = ""
    XMPP_JVB_COMPONENTS = 10
...
module "mig-haproxy" {
  ...
  startup_script    = "${data.template_file.jitsi-shared.rendered}\n ${data.template_file.jitsi-haproxy-startup-script.rendered}"
}
...
module "mig-meet" {
  ...
  startup_script    = "${data.template_file.jitsi-shared.rendered}\n ${data.template_file.jitsi-meet-startup-script.rendered}"
}
...
module "mig-jvb" {
  ...
  startup_script    = "${data.template_file.jitsi-shared.rendered}\n ${data.template_file.jitsi-jvb-startup-script.rendered}"
}
